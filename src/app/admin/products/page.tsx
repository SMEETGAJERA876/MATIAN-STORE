...
export default function AdminProductsPage() {
  const { products, addProduct, deleteProduct, toggleStock } = useProductStore();
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    await addProduct({
      ...
      features: ["Advanced Formula", "Eco-friendly"],
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium mt-1">
            Manage inventory products and availability
          </p>
        </div>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-[#1E293B] max-w-lg w-full rounded-3xl border border-slate-800 p-6 space-y-5">
            <h3 className="text-lg font-extrabold text-white">Add Product</h3>
            <form onSubmit={handleCreateProduct} className="space-y-4 text-xs">
              ...
              <div className="flex gap-3 pt-3">
                <button
                  ...
                  className="flex-1 rounded-xl bg-slate-800 py-3 font-bold text-slate-300"
                >
                  Cancel
                </button>
                <button
